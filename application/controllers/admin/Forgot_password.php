...
class Forgot_password extends CI_Controller{
	...
	public function forgot_password(){
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            if($this->form_validation->run() != false) {
                $check = get_table_value('tbl_admin', array('email' => $this->input->post('email')));
                if(empty($check)){
                    $this->session->set_flashdata('error', "Hello Admin, This is not correct email ");
                    ...
                    $message = $this->load->view('admin/mailer/customer-reset-password', $mail_data, true);
    ...
    public function reset_password(){
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            if($this->form_validation->run() != false) {
                $check = get_table_value('tbl_admin', array('id' => $post['id'], 'forgot_password_link' => $post['link']));
                if(empty($check)){
        ...
        $check = get_table_value('tbl_admin', array('forgot_password_link' => $this->uri->segment(3)));
        $data['id'] = $check['id']; 
